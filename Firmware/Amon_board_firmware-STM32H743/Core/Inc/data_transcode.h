...
typedef struct {
	int8_t 	edf_pwr_percent;		// Power percent for EDF PWM
	int8_t 	x_plus_angle;			// Angle for x+ servo PWM
	int8_t 	x_minus_angle; 			// Angle cycle for x- servo PWM
	int8_t 	y_plus_angle;			// Angle cycle for y+ servo PWM
	int8_t 	y_minus_angle;			// Angle cycle for y- servo PWM

} s_calib_data_packet;


typedef struct {
    s_boot_packet   	boot_packet;	// Bootloader packet frame
    s_rf_packet     	rf_packet;		// RF packet frame: drone->PC
    s_rf_packet_drone 	rf_packet_drone;// RF packet frame: PC->drone
    s_uart_packet   	uart_packet;	// UART packet frame
    s_calib_data_packet calib_data;		// UART packet data for calibration
