...
#define SPI_IS_BUSY 					(HAL_GPIO_ReadPin(CS_F_GPIO_Port, CS_F_Pin) == GPIO_PIN_RESET)
...
#define SET_FLASH_SPI_BAUDRATE 			FLASH_SPI->CFG1 &= ~SPI_CFG1_MBR_Msk; \
										FLASH_SPI->CFG1 |= FLASH_PRESCALER
...
void Flash_Select(void) {
		while (SPI_IS_BUSY) {}
		//SET_FLASH_SPI_BAUDRATE; // Use of separate bus - not needed
		HAL_GPIO_WritePin(CS_F_GPIO_Port, CS_F_Pin, GPIO_PIN_RESET);
}
...
void Flash_UnSelect(void) {
	...
	HAL_GPIO_WritePin(CS_F_GPIO_Port, CS_F_Pin, GPIO_PIN_SET);	//unselect
...
void Flash_Receive(uint8_t* data, uint16_t dataSize){
	HAL_SPI_Receive (&FLASH_SPI_PORT , data, dataSize, HAL_MAX_DELAY); // HAL_MAX_DELAY, 10
}
...
void Flash_Polling_Transmit(uint8_t* data, uint16_t dataSize){
	HAL_SPI_Transmit(&FLASH_SPI_PORT , data, dataSize, HAL_MAX_DELAY); // HAL_MAX_DELAY, 10
}
